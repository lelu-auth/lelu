"""
Prism integration for CrewAI agents.

Provides ``PrismTool`` — a CrewAI-compatible tool base class that gates every
``_run()`` call through Prism's Confidence-Aware Auth before execution.
...
.. code-block:: python
    ...
    tool = RefundTool(prism_client=client)
...
- If Prism denies the action, ``_run()`` returns a structured refusal string
...
class PermissionDeniedError(Exception):
    """Raised by PrismTool when ``throw_on_deny=True`` and Prism denies."""
...
class PrismTool(CrewAIBaseTool):  # type: ignore[misc]
    """CrewAI ``BaseTool`` with Prism Confidence-Aware Auth.

    Subclass this and implement ``_execute()`` instead of ``_run()``.
    Every call to ``_run()`` is intercepted and gated through Prism first.

    Attributes
    ----------
    actor:
        The Prism agent scope / actor name registered in your ``auth.yaml``.
    action:
    ...
    # Prism-specific fields (Pydantic v2 model fields)
    actor: str = Field(..., description="Prism agent actor / scope name")
    ...
    def __init__(self, prism_client: LeluClient, **data: Any) -> None:
        ...
        super().__init__(**data)
        self._prism = prism_client

    # ── Intercept CrewAI's _run ───────────────────────────────────────────────

    def _run(self, *args: Any, **kwargs: Any) -> str:
        """
        Gate the tool call through Prism before executing ``_execute()``.
        Called by CrewAI's agent loop automatically.
        """
        try:
            decision = asyncio.run(
                self._prism.agent_authorize(
                    AgentAuthRequest(
                        actor=self.actor,
                        action=self.action,
            ...
            msg = f"[Prism] Authorization check failed for '{self.name}': {exc}"
            logger.error("prism: %s", msg)
        ...
        if decision.requires_human_review:
            logger.info(
                "prism: tool=%s queued for human review reason=%r",
                self.name,
                decision.reason,
            )
            return (
                f"[Prism] Action '{self.name}' is queued for human review. "
        ...
        if not decision.allowed:
            msg = (
                f"[Prism] Action '{self.name}' was denied for agent '{self.actor}'. "
        ...
        logger.debug(
            "prism: tool=%s authorized confidence=%.2f trace_id=%s",
            self.name,
    ...
    def _execute(self, *args: Any, **kwargs: Any) -> str:
        ...
        raise NotImplementedError(
            f"PrismTool subclass '{type(self).__name__}' must implement _execute()"
        )
