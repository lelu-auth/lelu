export default function DocsAdaptersPostgres() {
  return (
      ...
      <div className="space-y-12">
        ...
        <section>
          ...
          <p className="text-zinc-600 dark:text-zinc-400 mb-4">
            Make sure PostgreSQL is running and accessible. Pass the connection string as an environment variable to the Platform service.
          </p>
        ...
        <section>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  ...
                  <tr key={feat} className="border-b border-zinc-100 dark:border-zinc-800 last:border-0">
          ...
          <p className="text-zinc-600 dark:text-zinc-400 mb-4">
            To add a new column, append an idempotent <code className="font-mono text-sm px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">ALTER TABLE</code> statement to the migrations slice in <code className="font-mono text-sm px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">internal/db/db.go</code> and restart the Platform.
          </p>
          <div className="bg-zinc-900 dark:bg-black rounded-xl border border-zinc-800 dark:border-white/10 overflow-hidden">
    ...
    \`CREATE INDEX IF NOT EXISTS idx_policies_tenant
        ...
        {/* Tables */}
        <section>
          <h2 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-4">Schema Overview</h2>
          <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300">Table</th>
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300">Description</th>
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300">Key columns</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ["policies",      "OPA policy bundles",                      "id, tenant_id, content, version"],
                  ["audit_trails",  "Immutable HMAC-signed decision log",       "trace_id, decision, hmac, reviewer"],
                  ...
                  <tr key={table} className="border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                    <td className="px-4 py-3 font-mono text-xs text-zinc-800 dark:text-zinc-200">{table}</td>
                    <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{desc}</td>
                    <td className="px-4 py-3 font-mono text-xs text-zinc-500 dark:text-zinc-500">{cols}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
ALTER DEFAULT PRIVILEGES IN SCHEMA lelu GRANT ALL ON TABLES TO lelu;`}</pre>
        ...
        <section>
          ...
          <div className="space-y-3">
            {[
              { rule: "Add columns as nullable or with DEFAULT",   detail: "PostgreSQL 11+ can add NOT NULL + DEFAULT columns without a full table rewrite." },
              { rule: "Never rename/drop in a single deploy",       detail: "Add the new column → backfill → update app code → drop old column across separate deploys." },
              { rule: "Create indexes CONCURRENTLY",                detail: "Prevents locking reads/writes on large tables during index creation." },
              { rule: "Back up before every migration",             detail: "Run pg_dump before applying changes so you can restore if something goes wrong." },
            ].map((item) => (
...
psql $DATABASE_URL < backup_20260223_120000.sql`}</pre>
        ...
        <section>
          ...
          <p className="text-zinc-600 dark:text-zinc-400">
            For performance tuning, connection pooling with <strong>PgBouncer</strong> (transaction mode) is recommended in high-throughput deployments. Point read-heavy endpoints such as the audit trail API to a <strong>read replica</strong> to offload the primary.
          </p>
        </section>
      ...
      <div className="flex justify-between items-center pt-12 mt-12 border-t border-zinc-200 dark:border-white/10">
        ...
        <a href="/docs/databases/redis" className="inline-flex items-center gap-2 text-sm font-medium text-zinc-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
          Next: Redis
