  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="space-y-12">
        ...
        <section>
          <h2 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-6">PostgreSQL (Platform)</h2>
          <p className="text-zinc-600 dark:text-zinc-400 mb-4">The Platform service uses PostgreSQL to persist all long-lived data. The schema is managed by the Platform startup code — no separate migration tool is required.</p>
        ...
        <section>
          <h2 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-6">Redis (Engine Queue)</h2>
          <p className="text-zinc-600 dark:text-zinc-400 mb-4">The Engine uses Redis as a queue and cache for in-flight authorization requests, confidence scores, and human-in-the-loop polling state.</p>
          ...
          <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                  <th className="px-4 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-300">Pattern</th>
        ...
        <section>
          <h2 className="text-2xl font-semibold text-zinc-900 dark:text-white mb-4">Production Tips</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { title: "Connection pooling", desc: "Set max_connections in PostgreSQL and use PgBouncer in transaction mode for the Platform service." },
              { title: "Redis persistence", desc: "Enable AOF persistence (appendonly yes) so pending approvals survive a Redis restart." },
              { title: "Read replicas", desc: "Point audit trail read API endpoints to a PostgreSQL read replica to avoid locking the primary." },
              { title: "TLS", desc: "Use sslmode=require in the DATABASE_URL and rediss:// (TLS) for the Redis URL in production." },
            ].map((tip) => (
              <div key={tip.title} className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
                <h4 className="font-semibold text-zinc-900 dark:text-white text-sm mb-1">{tip.title}</h4>
      ...
      <div className="flex justify-between items-center pt-12 mt-12 border-t border-zinc-200 dark:border-white/10">
        <a href="/docs/integrations/mobile" className="inline-flex items-center gap-2 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors">
